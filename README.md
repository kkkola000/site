...
**Про `--attach-site`.** Скрипт находит работающий сайт nginx (по `proxy_pass` на порт
соседней панели), добавляет в него блок `location /anex-orders/` между маркерами,
делает резервную копию, проверяет `nginx -t` и откатывает файл, если проверка не прошла.
