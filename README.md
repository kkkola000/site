...
правил, не трогая чужой. Список можно задать явно:
...
Если сети найти не удалось, скрипт скажет об этом прямо, а не сделает вид,
что панель закрыта: вход останется под паролем панели.
...
|---|---|
| `YANDEX_OAUTH_TOKEN` | OAuth-токен Яндекс Доставки (обязателен) |
