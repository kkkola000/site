...
3. **Список заказов**, сгруппированный по дате доставки. Строка заказа — пять столбцов:
   * название компании (из настроек), трек-номер с кнопкой копирования и номер заказа;
   * товары и количество;
   * стоимость страховки (оценочная стоимость товаров);
   * способ и адрес доставки;
   * последний статус и кнопка «Печать» — ярлык в формате по умолчанию;
   * шкала этапов: Ожидает отгрузки → В пути → Готов к вручению → Выдан.
     Пройденные этапы заливаются синим, текущий выделен, будущие серые. Под
     каждым этапом — время, когда заказ до него дошёл, а у последнего, пока
     заказ не выдан, плановая дата доставки («≈ 17 сент»). На телефоне та же
     шкала разворачивается в столбец. На телефоне шкала и кнопка печати в
     списке скрыты — и то и другое остаётся в карточке заказа.

Клик по карточке открывает подробности: история статусов, получатель, маршрут,
грузоместа, ссылка на трекинг для получателя и печать ярлыка с выбором формата.

Адреса отгрузки и доставки показываются в подробностях заказа, а не в списке.
...
Рядом кнопки «Проверить связь» и «Удалить токен».

Там же выбирается **формат ярлыка по умолчанию** — из тех, что принимает
`generate-labels`: 58 × 40 (по умолчанию, термопринтер), 58 × 60, 75 × 120,
100 × 150 и A4 — . `generate_type` в запрос не добавляется: по
документации он задаёт раскладку на листе A4 и к размеру этикетки отношения
не имеет.
В карточке заказа формат можно поменять на разовую печать, не трогая настройку.

Пустое поле при сохранении означает «не менять токен». Чтобы убрать его, нужна
отдельная кнопка «Удалить токен» — после подтверждения панель возвращается к экрану
подключения и перестаёт обращаться к API. Если токен задан ещё и в `.env` сервера,
панель продолжит работать на нём и скажет об этом.
...
Яндекс Доставка требует TLS 1.2 и выше с шифрами по PCI DSS v4. Панель
фиксирует минимальную версию явно (`server/config.js`), набор шифров —
стандартный для Node, устаревших в нём нет.

Проверить фактическое соединение с сервера:

```bash
node /opt/anex-orders-src/tools/tls-check.mjs
```

Скрипт печатает согласованный протокол, шифр и срок действия сертификата.

## Проверка ярлыка

`tools/label-check.html` — отдельная страница, которая открывается локально
двойным щелчком, без сервера. Она собирает команду запроса к `generate-labels`
(curl или PowerShell) и измеряет страницу полученного PDF: видно, совпал ли
формат с запрошенным. Ничего никуда не отправляет — токен остаётся в браузере.

В саму панель проверка не встроена: это разовый инструмент, а не её функция.
...
`tools/label-check.html` — одна страница, которая открывается на компьютере
двойным щелчком. Поля: токен, request_ids, размер и раскладка; ниже — тело
запроса и кнопка. Ярлык сохраняется файлом, а рядом показывается размер
страницы полученного PDF в миллиметрах: сразу видно, совпал ли формат
с запрошенным.

Если браузер заблокирует запрос (CORS), страница покажет готовую команду curl.
...
Этапы шкалы совпадают с разделами панели: «Ожидает отгрузки», «В пути»,
«Готов к вручению», «Выдан». Возвратный заказ занимает место третьего этапа,
но подписывается «Возврат» — путь обратно виден, а длина шкалы не меняется.
Заказ, закрытый не выдачей, подписывается по факту: «Отменён» или
«Возвращён в магазин».
