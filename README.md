...
3. **Список заказов**, сгруппированный по дате доставки. В карточке:
   ...
   * шкала этапов: Создан → Принят в СЦ → В пути → Готов к вручению → Завершён.
...
На шкале этапов возвратный заказ занимает то же место, что и «Готов к вручению»,
но подписывается «Возврат» — путь обратно виден, а длина шкалы не меняется.
У завершённого заказа последний этап подписывается по факту: «Доставлен»,
«Отменён» или «Возвращён в магазин».
