...
100 × 150 и A4 — и **раскладка на листе A4** (`generate_type`): «один ярлык
на страницу» (`one`, по умолчанию) или «максимум ярлыков на странице» (`many`).
Для форматов этикеток этот параметр в запрос не добавляется — по документации
он относится только к A4.
