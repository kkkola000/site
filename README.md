...
3. **Список заказов**, сгруппированный по дате доставки. В карточке:
   ...
   * шкала статусов: на ПК горизонтальная — точка на каждое событие, подписаны
     первое, основное и текущее (текущее выделено); на телефоне остаются эти же
     три точки в столбец.
...
100 × 150 и A4 — и **раскладка на странице** (`generate_type`): «максимум
ярлыков на странице» (`many`, по умолчанию) или «один ярлык на страницу» (`one`).
